...
async def wallet_add(user_id: int, amount: int, kind: str, reference_id="", note=""):
    ...
    async with await db.connect() as conn:
        ...
        row = await (await conn.execute("SELECT balance FROM users WHERE id=%s", (user_id,))).fetchone()
        if not row:
...
async def create_order(user_id: int, payment_method: str, wallet_requested=True):
    async with await db.connect() as conn:
        await conn.execute("BEGIN")
        user = await (await conn.execute("SELECT * FROM users WHERE id=%s", (user_id,))).fetchone()
