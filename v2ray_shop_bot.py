...
A Telegram V2Ray shop bot built around aiogram 3 + aiohttp + PostgreSQL (psycopg 3).
...
DATABASE_URL              PostgreSQL connection URL (required on Render)
...
Install:
pip install aiogram aiohttp "psycopg[binary,pool]"
...
class DB:
    ...
    async def init(self):
        ...
        # Telegram user IDs can exceed PostgreSQL INTEGER (32-bit).
        async with await self.connect() as db:
            async with db.cursor() as cur:
                await cur.execute("ALTER TABLE users ALTER COLUMN telegram_id TYPE BIGINT USING telegram_id::BIGINT")
                await cur.execute("ALTER TABLE users ALTER COLUMN id TYPE BIGINT USING id::BIGINT")
                await cur.execute("ALTER TABLE users ALTER COLUMN referred_by TYPE BIGINT USING referred_by::BIGINT")
            await db.commit()
        await self.seed()
    ...
    async def seed(self):
        async with await self.connect() as db:
            async with db.cursor() as cur:
                await cur.execute("SELECT COUNT(*) AS c FROM categories")
                ...
                await cur.execute("SELECT COUNT(*) AS c FROM products")
                pcount = (await cur.fetchone())["c"]
                if pcount == 0:
                    await cur.execute("SELECT id,name FROM categories")
                    rows = await cur.fetchall()
                    cats = {row["name"]: row["id"] for row in rows}
                    ...
                    if SEED_DEMO:
                        await cur.execute("SELECT id FROM products")
                        product_ids = [r["id"] for r in await cur.fetchall()]
    ...
    async def run(self, query, params=()):
        """Execute one statement and return the inserted id when available."""
        async with await self.connect() as db:
            normalized = query.lstrip().lower()
            if normalized.startswith("insert into") and "returning" not in normalized and "into settings" not in normalized:
                query = query.rstrip().rstrip(';') + " RETURNING id"
            cur = await db.execute(query, params)
            row = await cur.fetchone() if normalized.startswith("insert into") and "into settings" not in normalized else None
            await db.commit()
    ...
    async with await db.connect() as conn:
        ...
        for item in items:
            for _ in range(item["quantity"]):
                ...
                if cur.rowcount != 1:
                    raise RuntimeError("Configuration was no longer available.")
...
async def receive_receipt(message: Message, state: FSMContext):
    ...
    existing = await db.one(
        "SELECT id FROM payments WHERE order_id=%s AND status='PENDING' LIMIT 1",
        (oid,),
    )
    if existing:
...
async def cb_cancel_order(c: CallbackQuery):
    ...
    if not order or order["user_id"] != (await db.one("SELECT id FROM users WHERE telegram_id=%s", (c.from_user.id,)))["id"]:
...
async def adm_backup(c: CallbackQuery):
    ...
    try:
        tables = [
            "users", "categories", "products", "configs", "cart_items",
            "coupons", "coupon_uses", "orders", "order_items", "payments",
            "wallet_transactions", "referrals", "tickets", "ticket_messages",
            "favorites", "admin_logs", "settings",
        ]
        payload = {"created_at": now_iso(), "database": "postgresql", "tables": {}}
        async with await db.connect() as src_conn:
            for table in tables:
                cur = await src_conn.execute(f"SELECT * FROM {table}")
                payload["tables"][table] = [dict(row) for row in await cur.fetchall()]
        target.write_text(json.dumps(payload, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        await audit(c.from_user.id, "BACKUP_CREATED", str(target), "PostgreSQL JSON export")
        await c.message.answer_document(
            document=FSInputFile(target),
            caption=f"💾 بکاپ PostgreSQL ساخته شد\n<code>{esc(target.name)}</code>",
        )
    except Exception as e:
