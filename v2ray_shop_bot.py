...
A Telegram V2Ray shop bot built around aiogram 3 + aiohttp + aiosqlite.
Designed for Render Web Service with persistent disk:
...
DATABASE_PATH             SQLite path (default: /var/data/v2ray_shop.db)
...
DATABASE_PATH = os.getenv("DATABASE_PATH", "/var/data/v2ray_shop.db")
...
class DB:
    ...
    async def seed(self):
        async with await self.connect() as db:
            ...
            if pcount == 0:
                cats = {}
                async with db.execute("SELECT id,name FROM categories") as c:
                    async for row in c:
                ...
                # Demo inventory is opt-in only. Production starts with zero stock.
                if SEED_DEMO:
                    async with db.execute("SELECT id FROM products") as c:
                        product_ids = [r["id"] async for r in c]
...
async def get_or_create_user(tg_user, referral_code: Optional[str] = None):
    row = await db.one("SELECT * FROM users WHERE telegram_id=?", (tg_user.id,))
    if row:
        await db.run(
        ...
        return await db.one("SELECT * FROM users WHERE telegram_id=?", (tg_user.id,))
    ...
    if referral_code and referral_code.startswith("ref_"):
        try:
            ...
            if inviter != tg_user.id:
                inv = await db.one("SELECT id FROM users WHERE telegram_id=?", (inviter,))
                if inv:
                    referred_by = inv["id"]
        except ValueError:
            pass
    ...
    return await db.one("SELECT * FROM users WHERE id=?", (uid,))
...
async def wallet_add(user_id: int, amount: int, kind: str, reference_id="", note=""):
    ...
    async with await db.connect() as conn:
        ...
        row = await (await conn.execute("SELECT balance FROM users WHERE id=?", (user_id,))).fetchone()
        if not row:
            raise ValueError("User not found.")
        before = row["balance"]
        after = before + amount
        await conn.execute("UPDATE users SET balance=?,updated_at=? WHERE id=?", (after, now_iso(), user_id))
        await conn.execute(
...
async def render_home(user_id: int):
    user = await db.one("SELECT * FROM users WHERE telegram_id=?", (user_id,))
...
async def cb_category(c: CallbackQuery):
    ...
    rows = await product_rows("p.active=1 AND p.category_id=?", (cid,))
...
async def cb_product(c: CallbackQuery):
    ...
    if not p:
        ...
        return
    fav = await db.one(
        "SELECT 1 FROM favorites f JOIN users u ON u.id=f.user_id WHERE u.telegram_id=? AND f.product_id=?",
        (c.from_user.id, pid),
    )
    text = (
        f"<b>{esc(p['name'])}</b>\n\n"
...
async def cb_buy_now(c: CallbackQuery):
    pid = int(c.data.split(":")[1])
    user = await db.one("SELECT id FROM users WHERE telegram_id=?", (c.from_user.id,))
    ...
    if not user or not product or not product["active"]:
        ...
        return
    stock = await db.one(
        "SELECT COUNT(*) AS c FROM configs WHERE product_id=? AND status='AVAILABLE'",
        (pid,),
    )
...
async def cb_cart(c: CallbackQuery):
    await c.answer()
    user = await db.one("SELECT id,balance FROM users WHERE telegram_id=?", (c.from_user.id,))
    items = await db.all(
...
async def cb_cart_change(c: CallbackQuery):
    ...
    pid = int(pid_s)
    user = await db.one("SELECT id FROM users WHERE telegram_id=?", (c.from_user.id,))
    item = await db.one("SELECT quantity FROM cart_items WHERE user_id=? AND product_id=?", (user["id"], pid))
    if item:
        if action == "cartplus":
            await db.run("UPDATE cart_items SET quantity=quantity+1 WHERE user_id=? AND product_id=?", (user["id"], pid))
        elif action == "cartminus":
            if item["quantity"] <= 1:
                await db.run("DELETE FROM cart_items WHERE user_id=? AND product_id=?", (user["id"], pid))
            else:
                await db.run("UPDATE cart_items SET quantity=quantity-1 WHERE user_id=? AND product_id=?", (user["id"], pid))
        else:
            await db.run("DELETE FROM cart_items WHERE user_id=? AND product_id=?", (user["id"], pid))
...
async def cb_fav(c: CallbackQuery):
    pid = int(c.data.split(":")[1])
    user = await db.one("SELECT id FROM users WHERE telegram_id=?", (c.from_user.id,))
    row = await db.one("SELECT id FROM favorites WHERE user_id=? AND product_id=?", (user["id"], pid))
    if row:
        await db.run("DELETE FROM favorites WHERE id=?", (row["id"],))
...
async def cb_favorites(c: CallbackQuery):
    ...
    rows = await db.all(
        ...
        WHERE u.telegram_id=? AND p.active=1 ORDER BY f.id DESC""",
        (c.from_user.id,),
    )
    await c.message.edit_text(
        "<b>❤️ علاقه‌مندی‌ها</b>\n\nانتخاب کنید:",
...
async def validate_coupon(user_id, code, subtotal):
    ...
    used = await db.one(
        "SELECT COUNT(*) c FROM coupon_uses WHERE coupon_id=? AND user_id=?",
        (coupon["id"], user_id),
    )
...
async def receive_coupon(message: Message, state: FSMContext):
    await state.clear()
    user = await db.one("SELECT id FROM users WHERE telegram_id=?", (message.from_user.id,))
...
async def create_order(user_id: int, payment_method: str, wallet_requested=True):
    async with await db.connect() as conn:
        ...
        setting = await (await conn.execute(
            "SELECT value FROM settings WHERE key=?", (f"cart_coupon:{user_id}",)
        )).fetchone()
        if setting:
            coupon_id = int(setting["value"])
            coupon = await (await conn.execute("SELECT * FROM coupons WHERE id=?", (coupon_id,))).fetchone()
        ...
        if coupon_id:
            ...
            await conn.execute("DELETE FROM settings WHERE key=?", (f"cart_coupon:{user_id}",))
...
async def complete_paid_order(order_id: int):
    async with await db.connect() as conn:
        await conn.execute("BEGIN IMMEDIATE")
        order = await (await conn.execute("SELECT * FROM orders WHERE id=?", (order_id,))).fetchone()
        if not order:
            raise ValueError("Order not found.")
        if order["status"] == "COMPLETED":
            await conn.commit()
            return []
        items = await (await conn.execute("SELECT * FROM order_items WHERE order_id=?", (order_id,))).fetchall()
        delivered = []
        for item in items:
            for _ in range(item["quantity"]):
                ...
                await conn.execute(
                    """UPDATE configs SET status='SOLD',sold_to=?,sold_order_id=?,sold_at=?
                       WHERE id=? AND status='AVAILABLE'""",
                    (order["user_id"], order_id, now_iso(), cfg["id"]),
                )
                delivered.append((item["product_name"], cfg["config"]))
        await conn.execute(
            """UPDATE orders SET status='COMPLETED',paid_at=COALESCE(paid_at,?),
...
async def send_delivery(order_id: int):
    order = await db.one("SELECT * FROM orders WHERE id=?", (order_id,))
    if not order:
        return
    user = await db.one("SELECT telegram_id FROM users WHERE id=?", (order["user_id"],))
    if not user:
...
async def cb_checkout(c: CallbackQuery):
    await c.answer()
    user = await db.one("SELECT * FROM users WHERE telegram_id=?", (c.from_user.id,))
    try:
...
async def receive_receipt(message: Message, state: FSMContext):
    ...
    user = await db.one("SELECT id FROM users WHERE telegram_id=?", (message.from_user.id,))
    order = await db.one("SELECT * FROM orders WHERE id=? AND user_id=?", (oid, user["id"]))
    if not order:
    ...
    if existing:
        await message.answer("ℹ️ این سفارش قبلاً یک رسید در انتظار بررسی دارد.")
        return
    pid = await db.run(
...
async def cb_cancel_order(c: CallbackQuery):
    ...
    if not order or order["user_id"] != (await db.one("SELECT id FROM users WHERE telegram_id=?", (c.from_user.id,)))["id"]:
...
async def cb_wallet(c: CallbackQuery):
    await c.answer()
    u = await db.one("SELECT * FROM users WHERE telegram_id=?", (c.from_user.id,))
    tx = await db.all(
        "SELECT * FROM wallet_transactions WHERE user_id=? ORDER BY id DESC LIMIT 5",
        (u["id"],),
    )
...
async def receive_wallet_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        return
    u = await db.one("SELECT * FROM users WHERE telegram_id=?", (message.from_user.id,))
    ...
    await message.answer(
        ...
        f"👤 به نام: {esc(owner)}\n\n"
        "پس از پرداخت، رسید را با دستور /receipt ارسال کنید یا از پشتیبانی کمک بگیرید.",
    )
...
async def cb_orders(c: CallbackQuery):
    await c.answer()
    u = await db.one("SELECT id FROM users WHERE telegram_id=?", (c.from_user.id,))
    rows = await db.all(
...
async def cb_referral(c: CallbackQuery):
    await c.answer()
    u = await db.one("SELECT * FROM users WHERE telegram_id=?", (c.from_user.id,))
    count = await db.one("SELECT COUNT(*) c FROM referrals WHERE inviter_id=?", (u["id"],))
    rewarded = await db.one(
        "SELECT COALESCE(SUM(reward_amount),0) s FROM referrals WHERE inviter_id=? AND rewarded=1",
        (u["id"],),
    )
    me = await bot.get_me()
...
async def reward_referral_if_needed(user_id: int):
    user = await db.one("SELECT * FROM users WHERE id=?", (user_id,))
    if not user or not user["referred_by"]:
        return
    ref = await db.one(
        "SELECT * FROM referrals WHERE invited_id=? AND rewarded=0", (user_id,)
    )
    if not ref:
        return
    ...
    await db.run(
        "UPDATE referrals SET rewarded=1,reward_amount=? WHERE id=?",
        (reward, ref["id"]),
    )
    inviter = await db.one("SELECT telegram_id FROM users WHERE id=?", (ref["inviter_id"],))
    if inviter:
        try:
            await bot.send_message(
                inviter["telegram_id"],
...
async def cb_profile(c: CallbackQuery):
    await c.answer()
    u = await db.one("SELECT * FROM users WHERE telegram_id=?", (c.from_user.id,))
    orders = await db.one("SELECT COUNT(*) c FROM orders WHERE user_id=?", (u["id"],))
    completed = await db.one("SELECT COUNT(*) c FROM orders WHERE user_id=? AND status='COMPLETED'", (u["id"],))
    await c.message.edit_text(
        f"<b>👤 حساب کاربری</b>\n\n"
...
async def ticket_message(message: Message, state: FSMContext):
    data = await state.get_data()
    await state.clear()
    u = await db.one("SELECT id FROM users WHERE telegram_id=?", (message.from_user.id,))
    tid = await db.run(
...
async def cb_mytickets(c: CallbackQuery):
    ...
    u = await db.one("SELECT id FROM users WHERE telegram_id=?", (c.from_user.id,))
    rows = await db.all(
        "SELECT * FROM tickets WHERE user_id=? ORDER BY id DESC LIMIT 10", (u["id"],)
    )
    if not rows:
...
async def adm_dashboard(c: CallbackQuery):
    ...
    today_rev = await db.one(
        "SELECT COALESCE(SUM(total),0) s FROM orders WHERE status='COMPLETED' AND completed_at LIKE ?",
        (today + "%",),
    )
    await c.message.edit_text(
        f"<b>📊 DASHBOARD</b>\n\n"
...
async def adm_payreject(c: CallbackQuery):
    ...
    order = await db.one("SELECT * FROM orders WHERE id=?", (p["order_id"],))
    if order:
        await db.run("UPDATE orders SET status='PAYMENT_REJECTED' WHERE id=?", (order["id"],))
        user = await db.one("SELECT telegram_id FROM users WHERE id=?", (order["user_id"],))
        if user:
...
async def receive_config(message: Message, state: FSMContext):
    ...
    p = await db.one("SELECT name FROM products WHERE id=?", (pid,))
    if not p:
        await message.answer("❌ محصول پیدا نشد.")
        return
    await db.run(
        "INSERT INTO configs(product_id,config,status,created_at) VALUES(?,?,?,?)",
...
async def ban_user(message: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    tid = int(parts[1])
    await db.run("UPDATE users SET is_banned=1 WHERE telegram_id=?", (tid,))
...
async def unban_user(message: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    tid = int(parts[1])
    await db.run("UPDATE users SET is_banned=0 WHERE telegram_id=?", (tid,))
...
async def addcoupon_cmd(message: Message):
    ...
    try:
        await db.run(
            """INSERT INTO coupons(code,kind,value,active) VALUES(?,?,?,1)""",
            (code, kind, value),
        )
...
async def adm_backup(c: CallbackQuery):
    ...
    target = backup_dir / f"v2ray_shop_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    try:
        async with await db.connect() as src_conn:
            async with aiosqlite.connect(str(target)) as dst_conn:
                await src_conn.backup(dst_conn)
        await audit(c.from_user.id, "BACKUP_CREATED", str(target), "")
        await c.message.answer(f"💾 بکاپ ساخته شد:\n<code>{esc(str(target))}</code>")
    except Exception as e:
